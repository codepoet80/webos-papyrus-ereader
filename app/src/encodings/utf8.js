	...
	for (var i = 0; i < bytes.length; i+=1) {
		...
		if (b <= 0x7F) {
			...
			if (i+1 >= bytes.length) {
				...
				//console.warn("Stream ended on 2 byte sequence. Ignoring character.");
				break;				
			} 
			...
			if (i+2 >= bytes.length) {
				droppedChars += bytes.length - i;
				//console.warn("Stream ended on 3 byte sequence. Ignoring character.");
				break;				
			}
			...
			if (i+3 >= bytes.length) {
				droppedChars += bytes.length - i;
				//console.warn("Stream ended on 4 byte sequence. Ignoring character.");
				break;				
			} 
			...
			console.warn("Invalid UTF-8 byte (range 0x80-0xBF) found. " +
				"Maybe this file is not UTF-8? Only reported once."
			);
		} else if (b >= 0xC0 && b <= 0xC1) {
			if (rC0C1reported) continue;
			console.warn("Invalid UTF-8 byte (range 0xC0-0xC1) found. " +
				"Maybe this file is not UTF-8? Only reported once."
			);
		} else {
			//The remaining space is invalid
			if (rRESTreported) continue;
			console.warn("Invalid UTF-8 byte (range 0xF5-0xFF) found. " +
				"Maybe this file is not UTF-8? Only reported once."
			);
		}
	}
