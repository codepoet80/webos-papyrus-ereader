/**
 * Translates an array of bytes from CP-936 to HTML Unicode escapes
 * @param {Object} bytes the array of bytes to convert.
 */
function translateCP936(bytes) {
	console.log("translateCP936");
	//At first we check if the cp936 translation table is okay
	if (cp936table == null) {
		//We decode the compressed table from cp936data
		var bytes = base64ToBytes(cp936data);
	...
	for (var i = 0; i < bytes.length; i+=1) {
		...
		if (code <= 0x7F) {
			...
			//The table only contains the two-byte codes; and stops at 0xFE4F
			if (code >= 0x8140 && code <= 0xFE4F) {
				//The code must be translated; a shift of 0 means an invalid char
				var shift = cp936table[code - 0x8140];
